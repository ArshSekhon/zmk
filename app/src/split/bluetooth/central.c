...
struct peripheral_slot {
    ...
    struct bt_gatt_discover_params sensor_discover_params;
...
static uint8_t split_central_sensor_desc_discovery_func(struct bt_conn *conn,
    ...
    if (!discovered && !bt_uuid_cmp(((struct bt_gatt_chrc *)attr->user_data)->uuid,
                     BT_UUID_DECLARE_128(ZMK_SPLIT_BT_CHAR_SENSOR_STATE_UUID))) {
		memcpy(&sensor_uuid, BT_UUID_GATT_CCC, sizeof(sensor_uuid));

        slot->sensor_discover_params.uuid = &sensor_uuid.uuid;
        slot->sensor_discover_params.start_handle = attr->handle + 1;
        slot->sensor_discover_params.type = BT_GATT_DISCOVER_DESCRIPTOR;
        slot->sensor_discover_params.func = split_central_sensor_desc_discovery_func;
        slot->sensor_subscribe_params.value_handle = bt_gatt_attr_value_handle(attr);
        err = bt_gatt_discover(conn, &slot->sensor_discover_params);
     ...
     else if(!bt_uuid_cmp((attr)->uuid, BT_UUID_GATT_CCC))
    {
        ...
        slot->sensor_subscribe_params.ccc_handle = attr->handle;
...
static uint8_t split_central_service_discovery_func(struct bt_conn *conn,
    ...
    memcpy(&sensor_uuid, BT_UUID_DECLARE_128(ZMK_SPLIT_BT_CHAR_SENSOR_STATE_UUID),
                sizeof(sensor_uuid));
    slot->sensor_discover_params.uuid = &sensor_uuid.uuid;;
    slot->sensor_discover_params.start_handle = attr->handle;
    slot->sensor_discover_params.end_handle = 0xffff;
    slot->sensor_discover_params.type = BT_GATT_DISCOVER_CHARACTERISTIC;
    slot->sensor_discover_params.func = split_central_sensor_desc_discovery_func;
    LOG_DBG("Beginning discovery for sensor service");
    err = bt_gatt_discover(conn, &slot->sensor_discover_params);
    if (err) {
        LOG_ERR("Discover failed (err %d)", err);
    }
    LOG_DBG("Succesfull discovery for sensor service!");
#endif /* ZMK_KEYMAP_HAS_SENSORS */
